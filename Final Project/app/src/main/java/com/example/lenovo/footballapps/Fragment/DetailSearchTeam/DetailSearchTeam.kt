package com.example.lenovo.footballapps.Fragment.DetailSearchTeam
...
import kotlinx.android.synthetic.main.activity_detail_search_team.*
...
class DetailSearchTeam : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_detail_search_team)

        val detail = intent.getParcelableExtra<SearchTeam>(DetailTeam.POSITION_EXTRA)
        id_Team = detail.idTeam.toString()

        getSupportActionBar()?.setTitle(detail.strTeam)

        Picasso.get().load(detail.strTeamLogo).into(img_detail_search_team)
        Picasso.get().load(detail.strTeamBadge).into(iv_detail_search_team_logo)
        Picasso.get().load(detail.strStadiumThumb).into(iv_detail_search_team_stadium)
        tv_detail_search_team.text = detail.strDescriptionEN

        iv_player_search_team.setOnClickListener {
            startActivity<PlayerTeam>("id_team" to detail.idTeam)
        }
