package com.example.lenovo.footballapps.Fragment.DetailTeam
...
class DetailTeam : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val detail = intent.getParcelableExtra<TeamData>(POSITION_EXTRA)
        id_Team = detail.idTeam.toString()

        getSupportActionBar()?.setTitle(detail.strTeam)

        Picasso.get().load(detail.strTeamLogo).into(img_detail_team)
        Picasso.get().load(detail.strStadiumThumb).into(iv_detail_team_stadium)
        Picasso.get().load(detail.strTeamBadge).into(iv_detail_team_logo)
        tv_detail_team.text = detail.strDescriptionEN

        iv_player_team.setOnClickListener {
